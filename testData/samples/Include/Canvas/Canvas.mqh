...
//--- if arc is obviously not within the rays range, don't draw
   ...
   if((fi3<M_PI_2 || fi3>3*M_PI_2) ||  // ëó÷ 3 ïîïàäàåò â 1 èëè 4 êâàäðàíò
      (fi<M_PI_2 || fi>3*M_PI_2)   ||  // ëó÷ 4 ïîïàäàåò â 1 èëè 4 êâàäðàíò
       fi4-fi3>=M_PI)                  // äóãà ïðîéäåò ÷åðåç ïðàâóþ ÷àñòü ýëëèïñà
     {
      dx=rx;
      dy=0;
      f=(ry_sq<<1)*((dx-1)*dx)+ry_sq+(rx_sq<<1)*(1-ry_sq);
